...
var fcCmds = newMenu("Focus Counter Menu Tool", newArray("Change channel to be measured", "Show channel to be measured", "-", "Select calibration images", "-", "Calibrate focus counter", "-", "Count foci and organize data", "Organize data only"));
...
macro "Working Directory Menu Tool - C037L02f2L5270L70c0Lc0f2L020cLf2fcL0cfc" {
	...
	if (noTempFile == false) {
		...
		showStatus(".." + File.separator() + workingPath[workingPath.length - 2] + File.separator() + workingPath[workingPath.length - 1] + File.separator() + " loaded.");
	}
}
...
macro "Clonogenics Configuration Action Tool - C037T0b10CT8b09fTdb09g" {
	if (File.exists(getDirectory("plugins") +
		...
		if (File.exists(getWorkingPaths("workingPath")) == true) {
			...
			showStatus("Working directory not found. Please set the working directory.");
		}
	} else {
		showStatus("Working directory has not been set.");
	}
		...
		if (File.exists(getWorkingPaths("workingPath")) == true) {
			if (File.exists(getWorkingPaths("analysisSetupFile")) == true) {
				...
				if (cmd == "Change channel to be measured") {
					...
					if (activeChannel > 0) {
						Dialog.addChoice("Count foci in:", labeledChoices, labeledChoices[activeChannel - 1]);
					} else {
						Dialog.addChoice("Count foci in:", labeledChoices, labeledChoices[0]);
					}
					Dialog.show();
		...
		if (File.exists(getWorkingPaths("workingPath")) == true) {
			if (File.exists(getWorkingPaths("analysisSetupFile")) == true) {
				if (File.exists(getWorkingPaths("obsUnitRoiPath")) == true) {
					...
					showStatus("OBS UNIT ROIs folder not found. Please select observational units.");
				}
			} else {
				showStatus("Setup file not found. Please run Clonogenics Configuration");
			}
