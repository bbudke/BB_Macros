...
macro "Observational Units Submasks Startup" {
	...
	if (choice == 1) {
		run("Install...", "install=[" + getDirectory("plugins") + "BB Macros" + File.separator() + "Clonogenics Modules" + File.separator() + "Observational Units Submasks.ijm]");
		cleanup();
	}
}
...
macro "Observational Units Submasks Configuration Action Tool - C037T0b10CT8b09fTdb09g" {
	...
	runMacro(getDirectory("plugins") + "BB Macros" + File.separator() + "Clonogenics Modules" + File.separator() + "Clonogenics Configurator.ijm", "change|3|1|" + radius);
}
...
function loadNextObsUnit() {
	...
	*/
	...
	if (zipIndex == -1) {
		...
		runMacro(getDirectory("plugins") + "BB Macros" + File.separator() + "Clonogenics Modules" + File.separator() + "Convert To Tiff.ijm", workingPath + zipListNoExt[zipIndex] + imageType + "|" + imageType + "|" + zSeriesOption);
