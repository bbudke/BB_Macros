	...
	*/
		...
		if (nFoci > 0) {
			...
			for (i=0; i<roiManager("Count"); i++) {
				...
				print(focusMeasurements, i + "\t" + pass + "\t" + focusArea + "\t" + focusAvgIntensity + "\t" + focusIntensity + "\t" + focusUpperDecile + "\t" + focusAreaTest + "\t" + focusAvgIntensityTest + "\t" + focusIntensityTest + "\t" + focusUpperDecileTest);
...
//			eval("script", frameScript("Foci mask.tif", 400, 400, 1200, 200));
			...
			if (nFoci > 0) {
				...
				print(resultsTable, "Focus\tPass\tArea\tAvg Int\tInt\tUpper Dec\tArea Diff\tAvg Int Diff\tInt Diff\tUpper Dec Diff");
	...
	*/
	...
	if (lengthOf(args) != 0) {
		...
		for (i=0; i<savedSettings.length; i++) {
			runMacro(getDirectory("plugins") + "BB Macros" + File.separator() + "Clonogenics Modules" + File.separator() + "Clonogenics Configurator.ijm", "change|4|" + toString((1 + i) + 8 * (channel - 1)) + "|" + toString(savedSettings[i]));
		}

		output = File.open(getDirectory("temp") + "FCC output.txt");
		print(output, "Focus Area\tFocus Avg Int\tFocus Int\tFocus Upper Dec");
		if (nBackendFoci > 0) {
			focusMeasurements = File.openAsString(getDirectory("temp") + "FCC foci temp.txt");
